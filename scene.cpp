...
void set_up_default_scene() {
  ...
  scene = add_sphere(scene, sphere3_ctr, sphere3_rad, sphere3_ambient,
		     ...
		     sphere3_reflectance, 1.5, 3);

  // chessboard
  if (chessboard_on)
  {
    //std::cout << "chessboard on" << std::endl;
    Point board_ctr = {0.0,1.5,-10.0};
    float board_amb[] = {1,1,1};
    float board_diff[] = {1, 1, 1};
    float board_spec[] = {0.5, 0.5, 0.5};
    float board_shineness = 30.0;
    float board_reflectance = 0.5;
    Vector board_normal = {0, -1, 0};
    chessboard = add_chessboard(chessboard, board_ctr, 1, board_amb,
      board_diff, board_spec, board_shineness, board_reflectance, 1,
      board_normal);
